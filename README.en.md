...
There are three installation paths. Pick the one that matches your purpose.

| Path | Docker required | Use for |
|---|---|---|
| **Prebuilt image** | no | Evaluation, quick start |
| **Private ECR in your account** | no (copy only) | Production |
| **Build from source** | yes | Contributing, modifying |

### Install from the prebuilt image (no Docker)
...
Container Registry, but that has nothing to do with your deployment region. The
gateway, the DynamoDB tables, the logs, and the Bedrock calls all live in your
account, in the region you pass to `--region` (`us-east-1` by default). Prompt
and response bodies are never stored in the first place.

The image contains no secrets. Only `src/` and the pinned Python dependencies go
in. The admin token is generated by CloudFormation at deploy time, stored in
Secrets Manager, and injected when the task starts. You can verify the image
contents from `Dockerfile` and `.dockerignore`, and every release carries a
GitHub-signed provenance attestation.

### Production: copy the image into your own private ECR

Fargate pulls the image every time it launches a task; there is no host cache.
An outage at an external registry therefore blocks scale-out and failure
recovery. For production, copy the image into your account and use that URI.
