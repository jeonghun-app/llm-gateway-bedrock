...
정렬 키를 요청 ID 로 둔 것이 **멱등성의 근거**다. 같은 요청을 두 번 기록하려
하면 `attribute_not_exists(sk)` 조건이 실패한다. 시간 역순 조회는 LSI 로 한다.
...
- **멱등성.** 조건이 실패하면 트랜잭션 전체가 취소되어 집계가 두 번 더해지지
  않는다. 재시도 안전성이 코드 로직이 아니라 저장소 제약으로 보장된다.
