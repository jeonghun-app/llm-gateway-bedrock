...
- **삭제된 팀·사용자를 참조하는 키를 fail-closed 로 거부한다.** 인증기가
  ...
  오류를 반환한다.
- **키 재발급 트랜잭션에 요청 토큰을 붙였다.** 관리 요청의 상관 ID 를
  `ClientRequestToken` 으로 전달해, boto3 의 SDK 내부 재시도가 트랜잭션을 두 번
  적용하지 않게 한다. 다만 HTTP `/rotate` 재호출은 여전히 새 평문 키를 새로
  발급한다(HTTP 수준 멱등성은 후속 과제).
...
- **관리 삭제를 조건부 삭제로 바꿨다.** 계정·팀·사용자 삭제가
