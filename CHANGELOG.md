...
- **키 재발급은 HTTP 수준에서 멱등하지 않다.** `ClientRequestToken` 은 SDK 내부
  ...
  발급된다. 완전한 HTTP 멱등성은 회전 결과를 작업 ID 별로 임시 저장하는 설계가
  필요하다.
- **관리 삭제에 조회-후-삭제 경쟁이 남아 있다.** 하위 리소스 조회와 부모 삭제
  사이에 자식이 생길 수 있다. 인증이 fail-closed 라 고아 키의 보안 위험은
  없지만, soft delete 또는 참조 레코드를 포함한 트랜잭션 삭제로 개선할 여지가
  있다.
