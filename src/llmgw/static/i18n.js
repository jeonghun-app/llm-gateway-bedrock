...
(function (global) {
   ...
   */
  const EN = {
    '기준선 수정: ': 'Edit baseline: ',
    '기준선 설정: ': 'Set baseline: ',
    ' 할 수 있다.': ' can do this.',
    '이 계정 전체가 가드레일 없이 동작하게 된다. 플랫폼 관리자만': 'The entire account will run without guardrails. Only a platform admin',
    ' 게이트웨이가 붙이면 클라이언트가 빼거나 바꿀 수 없다.': ' so having the gateway attach it means clients cannot remove or substitute it.',
    ' 콘솔에서 가드레일을 만들어도 호출마다 식별자를 실어야 적용되므로,': ' Creating a guardrail in the console is not enough: every call must carry the identifier,',
    'Amazon Bedrock Guardrails 를 게이트웨이가 모든 요청에 붙인다.': 'The gateway attaches Amazon Bedrock Guardrails to every request.',
    '미적용': 'Not applied',
    '개입': 'Intervened',
    '비우면 만료 없음. ISO-8601 UTC. 만료된 키는 401 이 되고 키는 남는다.': 'Leave empty for no expiry. ISO-8601 UTC. An expired key returns 401 but is retained.',
    '만료 시각': 'Expires at',
    '비우면 사용자 설정을 따른다.': 'Leave empty to inherit the user setting.',
    '비우면 제한 없음. 예산 초과폭을 줄이는 데 함께 쓴다.': 'Leave empty for no limit. Use together with budgets to bound overshoot.',
    '분당 요청 한도': 'Requests per minute',
    '해제': 'Remove',
    '삭제': 'Delete',
    '대상이 없다.': 'No subjects.',
    '숫자만. DRAFT 는 내용이 예고 없이 바뀌어 거부된다.': 'Numeric only. DRAFT is rejected because its contents can change without notice.',
    '버전': 'Version',
    'AWS 콘솔의 Guardrail ID': 'Guardrail ID from the AWS console',
    '가드레일 ID 또는 ARN': 'Guardrail ID or ARN',
    '왜 통제를 껐는지 남지 않으면 나중에 검토할 수 없다. 필수다.': 'Required. Without a recorded reason the exemption cannot be reviewed later.',
    '예: 레드팀 평가 (TICKET-1234)': 'e.g. red-team evaluation (TICKET-1234)',
    '사유': 'Reason',
    '면제를 해제한다. 이 대상은 다시 가드레일을 거친다.': 'Removes the exemption. This subject will pass through guardrails again.',
    '면제를 해제했다.': 'Exemption removed.',
    '면제했다.': 'Exemption granted.',
    '면제: ': 'Exempt: ',
    '면제 해제': 'Remove exemption',
    '적용': 'Applied',
    '면제': 'Exempt',
    '건. 면제에는 만료가 없으므로 정기적으로 검토한다.': ' — exemptions do not expire, so review them regularly.',
    '면제 ': 'Exemptions: ',
    '면제된 대상이 없다.': 'No exemptions.',
    '사용자 면제': 'User exemptions',
    '팀 면제': 'Team exemptions',
    '꺼짐': 'disabled',
    '기준선을 삭제했다.': 'Baseline deleted.',
    '기준선을 저장했다.': 'Baseline saved.',
    '기준선 삭제': 'Delete baseline',
    '기준선 수정': 'Edit baseline',
    '기준선 설정': 'Set baseline',
    '기준선이 없다. 이 계정은 가드레일 없이 동작한다.': 'No baseline. This account runs without guardrails.',
    '기준선 적용 중': 'Baseline active',
    '가드레일': 'Guardrails',
